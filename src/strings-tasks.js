...
function removeFirstOccurrences(str, value) {
  const start = str.slice(0, str.indexOf(value));
  const end = str.slice(str.indexOf(value) + value.length, str.length);
...
function removeLastOccurrences(str, value) {
  ...
  const end = str.slice(str.lastIndexOf(value) + value.length, str.length);
...
function isPalindrome(str) {
  const mergeStr = str.replace(/[\s.!,?%]/g, '').toLowerCase();
  let reverseStr = '';

  for (let i = mergeStr.length - 1; i >= 0; i -= 1) {
    reverseStr += mergeStr[i];
  }

  if (reverseStr === mergeStr) {
    return true;
  }
...
function findLongestWord(sentence) {
  const substrs = sentence.split(' ');
  const lengths = substrs.map((item) => item.length);
  const maxLength = Math.max(...lengths);

  return substrs.at(lengths.indexOf(maxLength));
}
